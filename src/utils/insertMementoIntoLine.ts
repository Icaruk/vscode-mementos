...
export function insertMementoIntoLine({
	...
	const insertMementoWithContext = config.get<boolean>("actions.insertMementoWithContext");
	...
	if (insertMementoWithContext) {
		...
		const _defaultContextTitle = config.get<string>("actions.defaultContextTitle") ?? "";
		...
		const previousLine = lineNumber === 0 ? null : activeEditor.document.lineAt(lineNumber - 1);
		const nextLine = lineNumber === lastLine ? null : activeEditor.document.lineAt(lineNumber + 1);
	...
	if (isEmptyLine) {
		activeEditor.edit(editBuilder => {
			if (activeEditor) {
				editBuilder.insert(activeEditor.selection.active, `${firstBlockToInsert}${contextText}`);
			};
		});
		
		// Place cursor
		activeEditor.selection = new Selection(
			new Position(lineNumber ?? 0, firstBlockToInsert.length - defaultContextTitle.length),
